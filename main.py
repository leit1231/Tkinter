...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_open_dialoge = tk.Button(toolbar, text='Добавить позицию', command=self.open_dialog, bg='#d7d8e0', bd=0, compound=tk.TOP, image=self.add_img)
        btn_open_dialoge.pack(side=tk.LEFT)
        ...
        btn_edit_dialoge = tk.Button(toolbar, text='Редактировать', bg='#d7d8e0', bd=0, image=self.update_img, compound=tk.TOP, command=self.open_update_dialoge)
        btn_edit_dialoge.pack(side=tk.LEFT)

        self.search_img = tk.PhotoImage(file='search.gif')
        btn_search_dialoge = tk.Button(toolbar, text='Поиск', bg='#d7d8e0', bd=0, image=self.search_img, compound=tk.TOP, command=self.open_search_dialoge)
        btn_search_dialoge.pack(side=tk.LEFT)

        self.refresh_img = tk.PhotoImage(file='refresh.gif')
        btn_refresh_dialoge = tk.Button(toolbar, text='Обновить', bg='#d7d8e0', bd=0, image=self.refresh_img,
                                       compound=tk.TOP, command=self.view_records)
        btn_refresh_dialoge.pack(side=tk.LEFT)
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.btn_add.bind('<Button-1>', lambda event: self.view.records(self.entry_description.get(),self.combobox.get(), self.entry_money.get()))
